...
**Last updated:** 2026-07-18 (after commit dfe6ec0). This is the authoritative status document
...
one remaining top-level Phase 5 bullet — two pages converted to the new template mechanism so far
(5.8.1 `admin_index`, 5.8.2 delete confirm), the rest is a straightforward continuation of that
pattern; everything else is either done or an optional, non-blocking follow-up (see the
...
3. **5.8 — History/audit, theming, extension points**: **5.8.1 and 5.8.2 (template engine
   mechanism, proven on `admin_index` and the delete-confirm page) are done** (commits `eb1a916`,
   `dfe6ec0`). **Next: 5.8.3+** — convert the remaining pages (changelist — the biggest and most
   complex, save for after a few more small ones prove the pattern further; add/change forms;
   bulk-delete confirm; list_editable's error page) one at a time the same careful "prove zero
   behavior change first" way, *then* real visual theming (CSS custom properties, dark mode,
   per-site branding) once every page renders through a template rather than `format!()`. This is
   also the natural point to revisit the CSRF header-only limitation (5.4's ledger entry) since
   server-rendered forms become the primary POST surface once theming replaces the plain
   `format!` HTML — and the point to add history/audit log entries.
