...
**Last updated:** 2026-07-18 (after commit b2c5d16). This is the authoritative status document
...
3. **5.8 — History/audit, theming, extension points**: **5.8.1 through 5.8.10 are done** (commits
   `eb1a916`, `dfe6ec0`, `965fb24`, `8b7dce5`, `55e54b5`, `ab1363e`, `cdd64bf`, `ad23e39`,
   `8a448db`, `b2c5d16`). Every admin page is template-based, themed, and brandable by title,
   heading, logo, and accent color; a real unmodified HTML `<form>` submission can now pass CSRF
   protection end-to-end, both at the core-middleware level (5.8.9) and wired all the way into
   the admin's own rendered `<form>` elements (5.8.10 Part A); default favicons and a manifest are
   served at every app's root regardless of admin mount prefix (5.8.10 Part B); optional
   `logo_url`/`accent_color` overrides exist per-`AdminSite` (5.8.10 Part C). **Next (not
   blocking): history/audit log entries** — a `LogEntry` model (user, action_time, app/model
   "content type" strings, object_id, `object_repr` via Django's generic
   `"<Model> object (<pk>)"` fallback convention, action_flag ADDITION/CHANGE/DELETION,
   change_message), wired into the 5 mutating admin handlers' success paths, scoped to v1 as a
   "Recent actions" panel on `admin_index.html` only — a per-object History page is a v2 cut.
   Already substantially investigated: the ORM's `inventory`-based auto-migration mechanism needs
   no bespoke migration for a new `#[derive(Model)]` struct; `require_perm` already returns the
   acting `User` at all 10 existing call sites in `djangors-admin/src/lib.rs` (currently all
   discarded via `.await?;` with no binding) so capturing the user id for logging is a minimal
   change; no existing `LogEntry`/`ContentType`/admin-history infra exists (the `djangors-auth`
   audit-signal tests are unrelated Phase-4 login/logout signals).
