...
**Last updated:** 2026-07-18 (after commit 55e54b5). This is the authoritative status document
...
one remaining top-level Phase 5 bullet — five pages converted to the new template mechanism so
far (5.8.1 `admin_index`, 5.8.2 delete confirm, 5.8.3 bulk-delete confirm, 5.8.4 list_editable
save-error page, 5.8.5 add/change form), leaving only the changelist page; everything else is
...
3. **5.8 — History/audit, theming, extension points**: **5.8.1 through 5.8.5 (template engine
   mechanism, proven on `admin_index`, the delete-confirm page, the bulk-delete confirm page,
   the `list_editable` save-error page, and the add/change form) are done** (commits `eb1a916`,
   `dfe6ec0`, `965fb24`, `8b7dce5`, `55e54b5`). Every admin page except the changelist itself is
   now template-based. **Next: 5.8.6** — convert the changelist (`admin_changelist`, the biggest
   and most complex page: columns, sorting, search, `list_filter`, `date_hierarchy`, pagination,
   `list_editable` inline inputs, CSV export link, bulk-delete/save-changelist `<form>` — do it
   last, now that every simpler page has proven the pattern). *Then* real visual theming (CSS custom properties,
   dark mode, per-site branding) once every page renders through a template rather than
   `format!()`. This is also the natural point to revisit the CSRF header-only limitation (5.4's
   ledger entry) since server-rendered forms become the primary POST surface once theming
