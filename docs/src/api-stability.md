...
- It remains functional, with the deprecation warning, for **at least one full minor release
  cycle** before it can be removed in a subsequent minor or major release — never removed in the
  same release it was deprecated in.
...
A root `CHANGELOG.md` is maintained from `1.0` onward (it does not exist as of this writing — the
next release that touches the public API should create it, following the
[Keep a Changelog](https://keepachangelog.com/) format: `Added`/`Changed`/`Deprecated`/`Removed`/
`Fixed`/`Security` sections per version).
...
This is a **first pass**, not a complete stability contract: it covers 3 of the ~26 workspace
crates by design (the largest, most-used ones), doesn't yet have a published `CHANGELOG.md` (no
release has happened yet to populate one), and doesn't cover the contrib crates or CLI tooling at
all. Treat this page as the honest current state of the policy, not a claim that everything is
frozen — it will be extended as later freeze-review passes land.
