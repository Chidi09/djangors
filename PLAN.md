...
- [ ] djangors-rest: **done (v1 core + auth):** serializers derived from `ModelMeta` (`serialize`/`deserialize`, generic over any `Model`, relation fields as raw ids), `ViewSet<M>` (list/retrieve/create/update/destroy), `viewset_routes` router mounting, pagination (reuses `djangors_core::Paginator` from 7.2) *(8.1, commit `a3f2257`)*; session auth (reuses `djangors_auth::Auth<U>`), token auth (`AuthToken` model, crypto-random keys via `OsRng`), JWT auth (optional `jwt` Cargo feature, `jsonwebtoken`, signed against the existing `DjangorsSettings.secret_key`), and permission classes — **`IsAuthenticated` is now the default for every `viewset_routes`-mounted route**, `AllowAny` available as an explicit opt-out via `viewset_routes_with_permission` *(8.2, commit `e77c3cd`, closing 8.1's zero-access-control gap)*. **remaining:** throttling, filtering/ordering query-param integration with `QuerySet`, OpenAPI 3.1 generation, browsable API page — each a separate future 8.x slice.
