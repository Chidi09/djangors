...
- [ ] CSRF middleware: token generation, template tag, form + header validation, exempt marker. On by default.
...
- [ ] Change form: **done (v1):** add + edit pages, all fields, server-side validation with all-errors-at-once reporting, changelist row links *(5.4, commit 9f016fe)*. **remaining:** fieldsets, readonly_fields customization, raw_id/autocomplete FK widgets (with async search endpoint), M2M widgets, inlines (tabular/stacked) via formsets. *(known gap: CSRF header-only validation means a raw browser form POST to these pages needs client-side JS to work — tracked in the roadmap doc, not yet fixed)*
...
- [ ] Theming: clean modern default (CSS custom properties, dark mode); every template overridable; per-site branding (title, logo, colors) via settings. *(Conversion mechanism complete as of 5.8.6 — every admin page now renders through minijinja: `admin_index` (5.8.1, commit eb1a916), delete-confirm (5.8.2, commit dfe6ec0), bulk-delete confirm (5.8.3, commit 965fb24), `list_editable` save-error page (5.8.4, commit 8b7dce5), add/change form (5.8.5, commit 55e54b5), and the changelist — the biggest/most complex page, sort headers/pager/search/list_filter/date_hierarchy/CSV export all converted (5.8.6, commit ab1363e). Remaining: the actual CSS/dark-mode/branding work, now that there's a real template to skin.)*
