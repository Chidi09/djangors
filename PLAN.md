...
- [ ] Theming: clean modern default (CSS custom properties, dark mode); every template overridable; per-site branding (title, logo, colors) via settings. *(mechanism started: `djangors-template`'s new `TemplateEngine::from_embedded()` + one converted page (`admin_index`, byte-identical output proven) — 5.8.1, commit eb1a916. Remaining: convert every other admin page the same careful way, then the actual CSS/dark-mode/branding work.)*
