...
- [ ] LICENSE (MIT/Apache-2.0), CODE_OF_CONDUCT, CONTRIBUTING, SECURITY.md (with a security-report email), README with the pitch. **Partially done:** `LICENSE-MIT`, `LICENSE-APACHE`, and `README.md` exist. **Missing entirely:** `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, and `SECURITY.md`. The last is the one that matters most — a framework pitching itself at banking with no documented private security-report channel forces a would-be reporter to open a public issue.
...
- [ ] Security review checklist doc; threat model doc; fuzz targets for parsers (multipart, cookies, query strings). **Partially done.** Review doc exists (`docs/security-review-2026-07-27.md`). Fuzz targets exist for `cookie_header`, `query_string`, and `session_cookie_decode`, with empty `fuzz/artifacts/` (no crashes found). **Missing:** a **multipart fuzz target** — the one parser on this list handling the largest and least-structured untrusted input, and the one where a buffering implementation (see Phase 1) makes a resource-exhaustion bug most likely; and the **threat model doc**, which was never written. Fuzzing is also not run in CI, so the corpus only grows when someone runs it by hand.
...
- [ ] `djangors.rs` (or chosen domain) landing page: pitch, live admin demo (deployed e-commerce example, read-only), benchmark numbers, "deploy in 5 minutes" screencast. **Partially done.** The site is live at `djangors.vercel.app` (Astro + a prebuilt mdBook under `/docs/`) with the pitch, a `/benchmarks` page carrying the real measured numbers, `/compare`, and `/changelog`; mobile responsiveness was fixed on 2026-07-31 (Playwright-measured at 320/375/390/430/768/1024). **Missing:** the chosen domain (still a `vercel.app` subdomain), the **live read-only admin demo** — the single highest-conversion item on this line, since the admin is the whole pitch and there is currently no way to see it without building a project — and the screencast.
