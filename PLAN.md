...
- [ ] Delete confirmation: **done (v1):** confirm page + delete action, one-hop related-object counts via the `inventory`-based global model registry, `on_delete` shown for information only *(5.5, commit aba0ff9)*. **remaining:** transitive (multi-hop) related-object walk, real `on_delete` enforcement (currently metadata-only — see roadmap ledger); permission enforcement everywhere (view/add/change/delete, per object hook) *(blocked on groups/model-level permissions, deferred from Phase 4)*.
