...
- [ ] Changelist: **done:** all-fields columns, sorting, pagination *(5.2, commit ac647f9)*; `list_display` (real-field subset/reorder) + `search_fields` (ILIKE search box) via new `AdminSite::register_with()` customization API *(5.6.1, commit a6ad28f)*. **remaining:** `list_display` computed-method columns, `list_filter` (field filters + custom), `date_hierarchy`, `list_editable`, bulk actions (delete + custom + **CSV/XLSX export built-in**), saved views.
