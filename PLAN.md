...
- [ ] History: **done (v1):** admin log entries table (`LogEntry`, one entry per successful add/change/delete, per-user "Recent actions" panel on the admin index) *(5.8.11, commit `01cd081`)*. **remaining:** per-object History page, real field-level diffing/`change_message`, IP/request-id capture; deep integration with djangors-contrib-audit for full record diffing.
