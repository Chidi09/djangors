...
- [ ] Theming: clean modern default (CSS custom properties, dark mode — done, 5.8.7); every template overridable; per-site branding (title, logo, colors) via settings — remaining. *(Conversion mechanism complete as of 5.8.6 — every admin page renders through minijinja: `admin_index` (5.8.1, eb1a916), delete-confirm (5.8.2, dfe6ec0), bulk-delete confirm (5.8.3, 965fb24), `list_editable` save-error page (5.8.4, 8b7dce5), add/change form (5.8.5, 55e54b5), changelist (5.8.6, ab1363e). 5.8.7 (commit cdd64bf) added a real HTML5 page shell (`admin/base.html`) via minijinja `{% extends %}`/`{% block %}`, CSS custom properties, and automatic dark mode via `prefers-color-scheme` — every page now visually themed, not just template-rendered. Remaining: per-site branding via settings, deliberately deferred out of 5.8.7 to ship a clean non-configurable default first.)*
