...
- [ ] Theming: clean modern default (CSS custom properties, dark mode); every template overridable; per-site branding (title, logo, colors) via settings. *(mechanism proven on two pages: `admin_index` (5.8.1, commit eb1a916) and the delete-confirm page — the first with genuinely dynamic content, first real proof autoescaping protects it correctly (5.8.2, commit dfe6ec0). Both byte-identical to the pre-template output. Remaining: convert every other admin page the same careful way, then the actual CSS/dark-mode/branding work.)*
