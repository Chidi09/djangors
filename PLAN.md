...
- [ ] CSRF middleware: token generation, header validation **(done)**, form-field (`csrfmiddlewaretoken`) fallback validation **(done, core-level — 5.8.9, commit `8a448db`; admin `<form>` wiring is 5.8.10)**, template tag, exempt marker. On by default.
...
- [ ] Change form: **done (v1):** add + edit pages, all fields, server-side validation with all-errors-at-once reporting, changelist row links *(5.4, commit 9f016fe)*. **remaining:** fieldsets, readonly_fields customization, raw_id/autocomplete FK widgets (with async search endpoint), M2M widgets, inlines (tabular/stacked) via formsets. *(CSRF header-only gap fixed at the core-middleware level, 5.8.9, commit `8a448db` — a raw browser form POST with a `csrfmiddlewaretoken` field now works without client-side JS; the admin's own `<form>` elements don't emit that hidden field yet, tracked as 5.8.10)*
- [ ] Delete confirmation: **done (v1):** confirm page + delete action, one-hop related-object counts via the `inventory`-based global model registry, `on_delete` shown for information only *(5.5, commit aba0ff9)*; permission enforcement — `require_perm` gates GET/POST delete and bulk-delete on the `delete_{model}` codename, superusers bypass *(5.7.2, commit b9aed08)*. **remaining:** transitive (multi-hop) related-object walk, real `on_delete` enforcement (currently metadata-only — see roadmap ledger).
- [ ] Permissions: **done:** `Permission`/`Group`/`UserGroup`/`GroupPermission`/`UserPermission` models (plain FK join tables, no real many-to-many ORM support needed), `AuthUser::is_superuser()`, `has_perm(db, user_id, codename)`, `sync_permissions()` + `dj createpermissions` *(5.7.1, commit bb284a4)*; every admin view (changelist/export/add/change/list_editable save/delete/bulk-delete) now checks `has_perm` per action via `require_perm` instead of a blanket `is_staff` check, superusers bypass entirely, `admin_index` filters to only viewable models *(5.7.2, commit b9aed08)*. **remaining:** an admin UI for managing groups/permissions/user assignments (still raw SQL/CLI only); UI-level hiding of action buttons a user lacks permission for (enforcement is already real and server-side, just not hidden in the UI); `AuthUser`-generic (not hardcoded to `djangors_auth::User`) support, same standing limitation as the 5.1 staff gate.
- [ ] History: admin log entries table; deep integration with djangors-contrib-audit for full record diffing.
- [ ] Theming: clean modern default (CSS custom properties, dark mode — done, 5.8.7); every template overridable; per-site branding (title/heading — done, 5.8.8; logo, colors — remaining) via settings. *(Conversion mechanism complete as of 5.8.6 — every admin page renders through minijinja: `admin_index` (5.8.1, eb1a916), delete-confirm (5.8.2, dfe6ec0), bulk-delete confirm (5.8.3, 965fb24), `list_editable` save-error page (5.8.4, 8b7dce5), add/change form (5.8.5, 55e54b5), changelist (5.8.6, ab1363e). 5.8.7 (cdd64bf) added a real HTML5 page shell (`admin/base.html`), CSS custom properties, and automatic dark mode. 5.8.8 (ad23e39) added `SiteBranding` (`site_header`/`site_title`) threaded through `AdminSite::urls()`'s existing per-closure-snapshot mechanism into every template-rendering handler — `AdminSite::new()`'s signature unchanged, opt-in via `with_site_header`/`with_site_title` builders. Remaining: logo + per-site color overrides, a separate larger piece (needs a static-file/URL mechanism for the logo and a way to inject CSS custom-property overrides per site).)*
...
- [ ] Project-binary subcommands (via `Djangors::run()` dispatch): `migrate`, `makemigrations`, `createsuperuser`, `shell` (REPL offering the management-command environment; full evcxr integration as a stretch), `dbshell`, `test`, `collectstatic`, custom management commands API.
- [ ] djangors-test: `TestClient` (in-process requests, no network), per-test transactional DB rollback, test settings, fixtures (JSON/YAML loaddata + Rust factory helpers), `assertRedirects/assertTemplateUsed/assertContains` equivalents, snapshot testing for responses.
- [ ] Deployment story doc + generators: Dockerfile (distroless, ~20MB image), systemd unit, health-check endpoint, graceful shutdown, `djangors check --deploy`.

**DoD:** `djangors new mysite && cd mysite && djangors run` → welcome page in under 90 seconds on a clean machine (after crates cache); tutorial-app edit-to-reload under 3s.

### Phase 7 — Batteries (contrib) (~2–3 months, parallelizable, community-friendly)
- [ ] djangors-cache: API (`cache.get/set/get_or_set`), backends (in-memory/moka, Redis, database), per-view middleware, template fragment caching.
- [ ] djangors-mail: trait + SMTP (TLS), console, file, in-memory backends; HTML+text multipart helpers.
