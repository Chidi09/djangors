...
- [ ] Theming: clean modern default (CSS custom properties, dark mode); every template overridable; per-site branding (title, logo, colors) via settings. *(mechanism proven on five pages, all byte-identical to their pre-template output: `admin_index` (5.8.1, commit eb1a916), delete-confirm — first with genuinely dynamic content (5.8.2, commit dfe6ec0), bulk-delete confirm (5.8.3, commit 965fb24), `list_editable` save-error page (5.8.4, commit 8b7dce5), add/change form — first with genuine per-field-kind branching (5.8.5, commit 55e54b5). Remaining: the changelist (the one big remaining page, biggest and most complex), then the actual CSS/dark-mode/branding work.)*
